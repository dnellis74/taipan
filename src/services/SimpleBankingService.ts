...
export class SimpleBankingService implements BankingService {
    ...
    applyInterest(state: GameState): void {
        const interest = Math.floor(state.bank * SimpleBankingService.INTEREST_RATE);
        state.bank += interest;
    }

    applyDebtInterest(state: GameState): void {
        const interest = Math.floor(state.debt * SimpleBankingService.DEBT_INTEREST_RATE);
        state.debt += interest;
    }
} 